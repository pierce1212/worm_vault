...
[Autosar通信栈：I-PDU、N-PDU、L-PDU，要掰扯清楚](../AUTOSAR/UDS/Autosar通信栈：I-PDU、N-PDU、L-PDU，要掰扯清楚.md)
...
**`Dcm_Dsd_Prv_StateMachine()` 是 DCM 里“诊断服务分发和响应组织”的主状态机。DSL 负责收发传输，DSD 负责判断请求是否合法、调用具体 SID 服务、决定发正响应还是负响应。**
...
### 4.3 CanTp 重要配置(Transport Protocol)（传输层）
...
CanTp 是 **CAN Transport Protocol**，中文就是 **CAN 传输层协议**。

它的作用：**把超过一帧 CAN 能承载的数据拆开发送，再在接收端拼回去**。

因为普通 CAN 一帧最多 8 字节，CAN FD 一帧最多 64 字节，但 UDS 诊断数据可能很长，比如：

```
22 F1 86 F1 90 F1 8A ...
```

或者刷写数据几百字节、几千字节。这时候 CAN/CAN FD 自己不会帮你分包，所以需要 CanTp。
...
|功能|解释|
|---|---|
|分包|UDS 数据太长时拆成首帧 FF + 连续帧 CF|
|重组|接收方把多帧重新拼成完整 UDS 数据|
|流控|接收方用 FC 帧告诉发送方能不能继续发、发多快|
|加/去 PCI 头|给 CAN 数据加 `00/10/21/30` 这类控制字节，接收后再去掉|

常见 CanTp 帧类型：

|类型|名称|例子|作用|
|---|---|---|---|
|SF|Single Frame|`03 22 F1 86`|单帧就发完|
|FF|First Frame|`10 DE 62 F1 86 ...`|多帧第一帧|
|CF|Consecutive Frame|`21 ...`、`22 ...`|后续连续帧|
|FC|Flow Control|`30 08 0A ...`|接收方告诉发送方继续/等待/停止|
...
是 CAN FD 下的 Single Frame 长格式：

```
00 23 = CanTp PCI 头，表示后面 UDS 长度 35 字节
22 F1 86 ... = 真正的 UDS 请求
```
...
CanTp 去掉 PCI 后交给 DCM:
22 F1 86 ...
```

一句话总结：

**CanTp 是夹在 DCM 和 CAN 驱动之间的“分包/组包层”，让 UDS 这种长诊断数据能可靠地跑在 CAN/CAN FD 上。**
...
![](assets/2026.06.29%20AUTOSAR_Dcm_PduR_CanTp_CanIf_CanDrv诊断链路学习笔记/file-20260701144618972.png)
![](assets/2026.06.29%20AUTOSAR_Dcm_PduR_CanTp_CanIf_CanDrv诊断链路学习笔记/file-20260701144916980.png)

|配置项|当前值|作用|
|---|---|---|
|`CanTpRxTaType`|`CANTP_PHYSICAL`|表示这是物理寻址请求。最终 DCM 里会对应 `DCM_PHYSICAL_TYPE`。|
|`CanTpRxAddressingFormat`|`CANTP_STANDARD`|标准 CanTp 地址格式。没有额外地址字节，所以报文第 1 个字节就是 PCI。你的 `[00 23]` 里的 `00 23` 就是 CanTp PCI。|
|`CanTpRxNSduId`|`0`|CanTp 内部使用的 Rx NSdu 句柄，不是 CAN ID。|
|`CanTpRxNSduRef`|`UdsRxPhy_CanTp2PduR...`|CanTp 解包完成后，把完整 UDS 数据交给 PduR/DCM 的上层 PDU。|
|`CanTpRxPaddingActivation`|`CANTP_ON`|开启 padding 处理。CAN FD 未用满的字节会按 padding 规则处理。|
|`CanTpBs`|`8`|Block Size。ECU 如果收到多帧请求，会在 FC 里告诉 tester：最多连续发 8 个 CF，然后等下一个 FC。|
|`CanTpSTmin`|`0.02`|ECU 在 FC 里要求 tester 的连续帧间隔至少 20 ms。|
|`CanTpNar`|`0.07`|ECU 发送 FlowControl 帧后，等待 CanIf/CanDrv 发送确认的超时时间，70 ms。|
|`CanTpNbr`|`0.06`|ECU 收到 FF 或一个 block 后，准备并发出 FC 的时间限制，60 ms。|
|`CanTpNcr`|`0.15`|ECU 发完 FC 后，等待 tester 下一个连续帧 CF 的超时时间，150 ms。|
|`CanTpRxWftMax`|空|允许发送 FC.WAIT 的最大次数。空一般表示不用等待帧或走工具默认值。|
|`CanTpRbRxFcDisabled`|空|ETAS/RTA 供应商扩展项，和接收方向 FlowControl 行为有关，当前未显式配置。|
