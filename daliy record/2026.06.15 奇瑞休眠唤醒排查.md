...
# **<mark style="background: #FF5582A6;">CAN FD 报文现在也可以唤醒，我是否必须要启用 PN?</mark>**

是的，如果你的目标是“ECU 睡眠时只有 `0x600-0x6FF` 能唤醒，`0x99`、`0x117` 不能唤醒”，PN/Selective Wake 必须启用。

原因很直接：ECU 睡眠时 MCU 和 CAN Controller 不能正常收帧做 ID 判断，真正能在睡眠状态过滤 CAN ID 的只有收发器 TJA 1145。TJA 1145 的这种 ID/mask 过滤能力就是 PN/selective wake 功能。

不启用 PN 时，收发器只能做普通 bus wake：只要总线上有有效 CAN 活动，`0x99`、`0x117`、`0x616` 都可能触发 `CANTRCV_WU_BY_BUS`。这就是你现在看到的现象。

要区分两个层次：

- `CanTrcvPnEnabled = true`：必须。它让 TJA 1145 按 WUF ID/mask 判断唤醒。
- `CanNmPnEnabled / ComM PNC`：从硬件原理上不一定是“你想过滤 ID”的核心，但在你这个 RTA-BSW 工程里，工具校验要求 CanTrcv PN、CanNm PN、ComM PNC 配置一致，所以实际生成 BSW 时也要配好，否则会报你之前看到的那些 PN/PNC 错误。

所以结论：

```
只想任意 CAN 报文唤醒：PN 不需要。
想只允许 0x600-0x6FF 唤醒：PN 必须启用。
```

如果你不启用 PN，又想禁止 `0x99/0x117` 唤醒，软件层面最多只能“醒来以后再判断并重新睡”，但这已经不是“不能唤醒”了，电源和启动流程已经被触发了。
